/**
 * OpenCV kontur geometrisi — worker + (geçmiş) main ile birebir aynı mantık.
 * DOM: yalnız OffscreenCanvas (document.createElement yok).
 */
import type { PathRing } from "@/lib/editor/cutline/types";
import type { OpenCvMat, OpenCvModule } from "@/lib/editor/cutline/opencv-types";

export const OPENCV_MAX_EDGE_PX = 640;
...
export function generateOffsetPaths(
  cv: OpenCvModule,
  mask: OpenCvMat,
  offsetPx: number,
  smoothness: number,
...
export function scalePathsPx(paths: PathRing[], invScale: number): PathRing[] {
...
export function bitmapToCvMatScaled(
  cv: OpenCvModule,
  bitmap: ImageBitmap
  ...
  const w = Math.max(1, Math.round(natW * downscale));
  const h = Math.max(1, Math.round(natH * downscale));
  const canvas = new OffscreenCanvas(w, h);
  const ctx = canvas.getContext("2d");
  if (!ctx) throw new Error("OffscreenCanvas 2d yok");
  ctx.drawImage(bitmap, 0, 0, w, h);
  if (cv.matFromImageData) {
    return { mat: cv.matFromImageData(ctx.getImageData(0, 0, w, h)), downscale };
  }
  return { mat: cv.imread(canvas), downscale };
}

export function computePathsFromBitmap(
  cv: OpenCvModule,
  bitmap: ImageBitmap,
  offsetsMm: number[],
  smoothness: number,
