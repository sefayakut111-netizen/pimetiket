...
export type OdemelerPanelProps = {
  variant?: "page" | "embedded";
};

export function OdemelerPanel({ variant = "page" }: OdemelerPanelProps) {
    ...
    <>
      ...
      {mtdSummary && (
        <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          />
          <Kpi label="Bu ay brut siparis" value={fmt(mtdSummary.grossOrderTotal)} />
      ...
      {selected && (
        <Card padding="p-6" className="mt-6">
          ...
          <dl className="mt-4 grid gap-2 text-sm md:grid-cols-2">
            ...
            <Detail
              label="Durum"
              value={`${selected.status} (${selected.action})`}
            />
            <Detail label="Hata" value={selected.failureReason ?? "—"} />
          </dl>
  ...
  return (
    <main className="py-8 pb-20">
      <div className="mx-auto max-w-[1200px] px-6">{body}</div>
    </main>
  );
}
