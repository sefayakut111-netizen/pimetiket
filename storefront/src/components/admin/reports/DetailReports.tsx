/**
 * Detay raporlar — aylık trend, ürün kırılımı, popüler boyutlar.
 * /admin/finans "Detay Raporlar" sekmesinde kullanılır.
 */
...
export interface DetailReportsProps {
  orders: CustomerOrder[];
}

export function DetailReports({ orders }: DetailReportsProps) {
  ...
  return (
    <div>
      <p className="mb-5 text-base text-gri-700">
        {totalOrders} sipariş · {fmt(totalRevenue)} ₺ ciro · {fmt(totalQty)} adet
        basıldı
      </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          ...
          },
          {
            label: "İptal oranı",
            value: `%${cancelRate}`,
            accent: cancelRate > 5 ? "text-kirmizi" : "text-gri-700",
          },
        ].map((k) => (
          <Card key={k.label} padding="p-5">
            ...
            <div
              className={cn(
                "text-[28px] font-bold tabular-nums mt-1.5 leading-none",
                k.accent
              )}
            >
              {k.value}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card padding="p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-[12px] text-gri-500">{monthly.length} ay</span>
            </div>
          ...
          <Card padding="p-6">
            ...
            <div className="mt-6 pt-5 border-t border-gri-200">
              ...
              <div className="text-[28px] font-bold tabular-nums">{fmt(totalQty)}</div>
            </div>
          </Card>

          <Card padding="p-6" className="lg:col-span-2">
            <h2 className="text-lg font-semibold mb-4">Popüler boyutlar (Top 5)</h2>
            {sizes.length === 0 ? (
                      ...
                      <div className="h-2 bg-gri-100 rounded-full overflow-hidden">
                        <div
                          className={cn(
                            ...
                            s.product === "sticker" ? "bg-pim-mercan" : "bg-lacivert"
                          )}
                          style={{ width: `${w}%` }}
                        />
                      </div>
      ...
      <div className="mt-6 flex items-center gap-3 text-[12px] text-gri-500">
        <Icon.Info size={14} />
        <span>Tüm zamanlar — admin sipariş listesinden hesaplanır.</span>
      </div>
    </div>
  );
}
