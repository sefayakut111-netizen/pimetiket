 ...
 *
 * Blog kapak görseli — server multipart upload + magic-byte doğrulama.
 * Path: public-assets/blog/covers/{uuid}.{ext}
 */
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[admin/blog/upload-cover] sanitize:", err);
  ...
  const path = `blog/covers/${uuid}.${extForMime(claimedMime)}`;
  ...
  if (uploadErr) {
    console.error("[admin/blog/upload-cover]", uploadErr);
    return NextResponse.json(
