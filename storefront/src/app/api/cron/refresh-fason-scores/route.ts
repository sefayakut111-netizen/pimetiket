...
export async function GET(req: Request) {
  ...
  try {
    const payload = await withCronRun("refresh-fason-scores", async () => {
      const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
      const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
      if (!url || !serviceKey) {
        throw new Error("Supabase eksik");
      }
      ...
      return {
        summary: `${updates.length} fason skoru güncellendi`,
    ...
    console.error("[cron/refresh-fason-scores]", err);
