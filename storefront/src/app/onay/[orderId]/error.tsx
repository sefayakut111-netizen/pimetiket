...
export default function OnayError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    Sentry.captureException(error, { tags: { route: "onay" } });
    console.error("[onay] Unhandled error:", error);
  }, [error]);
    ...
    <main className="bg-gri-50 animate-fade-up min-h-[calc(100vh-64px)] py-12 flex items-center">
      <div className="mx-auto max-w-[560px] px-6 text-center">
        ...
        <h1 className="mt-3 text-[28px] md:text-[36px] font-semibold tracking-tight leading-tight">
          {isEn ? "Proof page hit a snag" : "Onay sayfasında bir sorun oluştu"}
        </h1>
        <p className="mt-3 text-base text-gri-700 leading-relaxed max-w-[440px] mx-auto">
          {isEn
            ? "Try again — your approval may still be saved. If it persists, contact us with the ref below."
            : "Tekrar dene — onayın kaydedilmiş olabilir. Devam ederse ref numarasıyla bize yaz."}
        </p>
