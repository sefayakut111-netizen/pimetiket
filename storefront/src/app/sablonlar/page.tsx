/**
 * Pim Etiket — /sablonlar (Lead Magnet)
 *
 * Ücretsiz şablon kütüphanesi — email karşılığında ZIP indirilir.
 *
 * Strateji:
 *   - 12 kategoride generic etiket şablonları (zeytinyağı, bal, kozmetik vb)
 *   - Email signup → POST /api/lead/subscribe → ZIP linki + welcome mail
 *   - PostHog event: lead_signup_completed
 *   - KVKK m.10 aydınlatma (kısa metin + uzun linki)
 *
 * Resend gelene kadar:
 *   - INSERT yapar (DB'ye düşer)
 *   - "Yarın mail atacağız" mesajı gösterir
 *   - downloadUrl env varsa direkt indirir
 */
...
const CATEGORIES: Array<{
  ...
  },
  {
    ...
    examples: "Botanik · Mermer · Pastel hat",
  },
  {
    id: "icecek",
    emoji: "🥤",
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        ...
        // PostHog event — consent yoksa no-op
        try {
          const { track } = await import("@/lib/analytics/posthog-events");
          // Reuse generic auth event since lead capture is similar shape
      ...
      <div className="mx-auto max-w-[1100px] px-4 md:px-8">
          ...
          <div>
            ...
            <ul className="mt-5 space-y-2 text-[14px]">
              {[
                ...
                <li
                  key={line}
                  className="flex items-start gap-2 text-gri-700"
                >
        ...
        <section className="mt-12 text-center bg-pim-mercan-tint rounded-2xl p-8">
          ...
          <p className="mt-2 text-[13.5px] text-gri-700 max-w-[520px] mx-auto leading-relaxed">
            Pim'e söyle, fikrini brief'e dönüştürelim. Daha sonra dilersen
            ekibimiz tasarımı hazırlasın.
          </p>
