...
export default function StickerConfiguratorError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    Sentry.captureException(error, { tags: { route: "sticker-yapilandir" } });
    console.error("[sticker/yapilandir] Unhandled error:", error);
  }, [error]);
    ...
    <main className="bg-gri-50 animate-fade-up min-h-[calc(100vh-64px)] py-12 flex items-center">
      <div className="mx-auto max-w-[560px] px-6 text-center">
        ...
        <h1 className="mt-3 text-[28px] md:text-[36px] font-semibold tracking-tight leading-tight">
          {isEn ? "Configurator hit a snag" : "Sticker konfigüratörde sorun oluştu"}
        </h1>
        <p className="mt-3 text-base text-gri-700 leading-relaxed max-w-[440px] mx-auto">
          {isEn
            ? "Try again — your cart draft may still be saved. If it persists, Pim is in the bottom-right."
            : "Tekrar dene — sepet taslağın kayıtlı olabilir. Devam ederse Pim sağ altta."}
        </p>
        ...
        <div className="mt-7 flex gap-3 justify-center flex-wrap">
          ...
          <Button variant="secondary" size="lg" href="/sticker">
            <Icon.Home size={16} /> {isEn ? "Stickers" : "Sticker"}
          </Button>
