#!/usr/bin/env node
/**
 * Migration 139 — tabaka sticker card images
 * Kullanım: node scripts/apply-migration-139.mjs [--dry-run]
 */
...
async function main() {
  ...
  const file = "139_sticker_tabaka_card_images.sql";
  ...
  const verify = await applySql(
    "verify",
    "SELECT key, image_src FROM product_cards WHERE key LIKE '%-sayfa%' ORDER BY key"
  );
