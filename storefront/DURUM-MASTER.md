...
- **✅ Uygulandı (Cursor + Claude verify):** (a) E2E iyileştirme — `_rsc` prefetch hataları ignore + sarım testine `toBeVisible()` hydration bekleme; (b) `ETIKET_STEP_NAMES` 4/5 → `winding_direction`/`winding_detail` (analytics); (c) **lock-chain refinement** — `use-sequential-steps.ts`: URL `?shape=` pre-fill artık görsel preselect, sonraki adımı AÇMIYOR (kullanıcı seçimi/touched gerekli). tsc temiz + kilit testi yeni assertion'larla geçti. Rapordaki P2/P3 URL-prefill endişesi kapandı.
- **Hâlâ açık (ürün kararı):** kiss-cut ayrı fiyat çarpanı; sepet "Düzenle" 10dk TTL expire'da toast (UX P2).
