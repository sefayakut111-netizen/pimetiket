...
- **🆕 Sepet fiyat tazeleme — mimari hazır, Cursor uygulayacak** (`CURSOR-PROMPT-SEPET-FIYAT-TAZELEME.md`): Sepet ürünü fiyatı (`unit`/`total`) ekleme anında donuyor; admin fiyat yayınlayınca eski ürün bayat kalıyor. Çözüm: **zaman bazlı silme DEĞİL** → fiyat-sürümü bazlı tazeleme. Her ürünün `pricedAt`'i, scope'unun `pricing_config.live_updated_at`'inden eskiyse `POST /api/cart/reprice` ile yeniden hesaplanır (checkout'un `validateCartPricing` recalc'ını reuse). Sepet kaybı yok, sadece fiyat değişince çalışır. Migration `priced_at` kolonu + `/sepet` & `/odeme` mount entegrasyonu + toast/rozet. Terk-edilmiş-sepet 30-60g cron ayrı/opsiyonel.
