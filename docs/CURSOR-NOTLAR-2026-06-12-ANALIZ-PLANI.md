...
## BÖLÜM 2 — SİSTEM MODÜLLERİ (NEREDE kontrol edeceğiz — 16 parça)

Sistem **16 analiz modülüne** bölündü. İlk 8 müşteri sipariş yolculuğu ekseninde (akış sırasıyla),
son 8 katman/cross-cutting ekseninde — böylece kopuk değil, bütünlüklü.
...
|---|---|---|---|---|---|---|
| **M9** | Kimlik, Oturum & RBAC | `auth`, `mfa/2fa`, `partner otp`, `auth-bridge`, `middleware`, `role`, `assert-permission`, `admin-rbac`, `impersonate`, `staff` | D6 D4 D3 | 🔴 | 🟡 kısmi (partner OTP bitti) | **P2** |
...
> **Admin paneli** ayrı modül değil: güvenliği M9 (RBAC), operasyonları ilgili akış modülünde
> (manuel sipariş/bypass-checkout → M2/M5, fason admin → M6, fiyat admin → M1) denetlenir.
...
| **P4** | **M15** AI/Ajanlar | En az akış-kritik (çoğu tavsiye niteliğinde) |
